    ...
    <div class="module-home-container">
      ...
      <main>
        <v-container class="generic-table-page">
          <!-- <h1>foo</h1>
        <div v-for="action in tableActions"
                  :key="action.icon">
                  <h1>FOOOOO</h1>
                  <h1>
                    {{action.text}}
              ...
              <v-card style="padding: 0px 5px;" class="rounded-sm">
                ...
                <v-data-table
                  ...
                  <template v-slot:item.option="{ item }">
                    ...
                    <v-btn text color="#f0ad4e" class="create-button">
                      <router-link
                        class="create-link"
                        :to="{name: 'UserList',
                        params: item.caseId }"
                      >
                      <h4 v-for="action in tableActions" :key="action.icon">
                        <v-icon v-model="actions" text>
                          {{action.icon}}
                        </v-icon>
                      {{action.text}}
                      </h4>
                      <!-- <h4>
                      <v-icon v-model="item.option" text >mdi-application-edit-outline</v-icon>
                      Data
                      </h4>
                      <h4>
                      <v-icon v-model="item.option" text >mdi-eye</v-icon>
                      View
                      </h4> -->
                      </router-link>
                    </v-btn>
...
export default {
  ...
  data() {
    return {
      ...
      tableActions: [
        { text: 'Data Entry', icon: 'mdi-application-edit-outline' },
      ...
      tableContent: {
        ...
        rows: [
          {
            caseNumber: 'BRCA001A',
            diagnosis: 'central portion of breast neoplasm',
            dateCreated: 'Nov 08, 2021',
            status: 'Active',
            caseId: '12312314',
          },
          {
